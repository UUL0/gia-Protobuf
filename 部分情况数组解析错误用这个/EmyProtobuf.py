...
# ===== 新增：embedded message 合法性校验 =====
def is_fake_embedded(msg):
    """如果embedded message全是空嵌套或无效数据，判定为伪装"""
    #if not isinstance(msg, dict) or len(msg) == 0:
    #    return True
    # 统计空字典占比
    #empty = sum(1 for v in msg.values() if isinstance(v, dict) and len(v) == 0)
    #if empty > len(msg) * 0.5:  # 超过一半是空嵌套
    #    return True
    # 检查是否有不合理的大浮点数（像你例子里的 e-289）
    #for v in msg.values():
    #    if isinstance(v, float) and abs(v) < 1e-100:
    #        return True

    # 4. 新增规则：检测无效字段号，proto规范里合法field_number最小为1，如果出现大量key里字段号为0的无效字段，直接判定是伪装
    zero_field_count = 0
    for key in msg.keys(): # 判断是否是不合法的'00:00:Varint'，00:一定不是00
        parts = key.split(':')
        if len(parts) >= 1:
            field_no = int(parts[0])  # 取第一个元素
            if field_no == 0:
                zero_field_count += 1
    # 无效0号字段占比超过10%直接判定为伪嵌套
    if len(msg) > 0 and zero_field_count / len(msg) > 0.1:  # ← 先判断len>0
        return True
    
    # 5. 新增规则：如果整个字典只有1个字符串/字节值，说明完全是repeated数组的原始内容被误读为独立字段
    # if len(msg) == 1:
    #    single_val = list(msg.values())
    #    if isinstance(single_val, (str, bytes)):
    #        return True
...
def ParseData(data, start, end, messages, depth = 0,V_Type = None):
    ...
    while start < end:
        ...
        if wire_type == 0x00:
            ...
            odV=V_Type[0] #缓存值而不是“对象”，我感觉应该是传了“指针”，不清楚python的=具体作用，AI解释是指针
            ...
            if ret == True and is_fake_embedded(messages['%02d:%02d:embedded message' % (field_number, ordinary)]):  # 强制标记为失败，走下面的repeated容错分支
                ret = False
                V_Type[0]=odV # 还原值
